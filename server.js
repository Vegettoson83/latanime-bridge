...
app.get("/extract", async (req, res) => {
  ...
  // Clear bad cached values (embed URLs stored as streams)
  const cached = cache.get(url);
  if (cached && Date.now() - cached.ts < CACHE_TTL) {
    const cachedUrl = cached.url;
    // Validate cached URL is actually a stream, not an embed page
    const isStream = cachedUrl.includes(".m3u8") || 
      (cachedUrl.includes(".mp4") && !cachedUrl.includes("/embed") && !cachedUrl.includes("embed-"));
    if (isStream) {
      console.log(`[cache] ${url} -> ${cachedUrl.slice(0, 80)}`);
      return res.json({ url: cachedUrl, cached: true });
    } else {
      cache.delete(url); // Remove bad cache entry
    }
  }
  ...
  try {
    ...
    // Route interception — most reliable
    await page.route("**/*", (route) => {
      ...
      if (!streamUrl) {
        if (u.includes(".m3u8")) {
          ...
          streamUrl = u;
        } else if (u.includes(".mp4") && !u.includes("/embed") && !u.includes("embed-") && u.includes("://") && !u.includes("analytics")) {
          // Only match .mp4 if it looks like a CDN file URL, not an embed page
    ...
    page.on("response", async (response) => {
      ...
      try {
        ...
        if (ct.includes("mpegurl") || ct.includes("x-m3u8")) {
          console.log(`[ct-m3u8] ${u.slice(0, 100)}`);
          streamUrl = u;
        }
      } catch {}
    });
